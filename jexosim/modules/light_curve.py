...
def run(opt):
  ...
  if opt.timeline.apply_lc.val ==1:  
    ...
    if opt.timeline.apply_lo_dens_LC.val == 1:
        ...
        opt.lc, opt.ldc = opt.planet.get_light_curve(opt, opt.cr, opt.cr_wl, 					 		 
        							opt.exosystem_params.primary_transit.val)
        ...
        highD_lc, opt.ldc = opt.planet.get_light_curveopt.planet.get_light_curve(opt, opt.cr, opt.cr_wl, 					 		 
        							opt.exosystem_params.primary_transit.val)
